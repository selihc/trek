...
func NewWrapper(pgDSN string, log trek.Logf, schema fs.FS, migrateOpts ...pgmigrate.OptionFn) (*Wrapper, error) {
	...
	log("postgresql version: %s", version)
...
func (w *Wrapper) Transact(ctx context.Context, txFn trek.TxFn) error {
	...
	if err != nil {
		w.log("error opening sql tx: %s", err)
		return err
	}
	...
	if err != nil {
		w.log("error within tx execution, rolling back: %s", err)

		rollBackErr := tx.Rollback()
		if rollBackErr != nil {
			w.log("error rolling back tx execution: %s", err)
